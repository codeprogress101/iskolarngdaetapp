...
class _RegisterPageState extends State<RegisterPage> {
  ...
  Future<void> _resendVerification() async {
    ...
    try {
      ...
      setState(() {
        _error =
            'Unable to resend confirmation right now. Please try again shortly.';
      });
    } catch (e) {
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _register() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        _error = _friendlyRegisterAuthError(e.message);
      });
      ...
      if (!mounted) return;
      setState(() {
        _error = 'Registration failed. Please try again in a moment.';
      });
    } finally {
      if (mounted) {
        setState(() {
  ...
  String _friendlyRegisterAuthError(String message) {
