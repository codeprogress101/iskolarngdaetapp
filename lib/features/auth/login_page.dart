...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<void> _resendVerification({required bool automatic}) async {
    ...
    try {
      ...
      setState(() {
        _error =
            'Unable to resend verification right now. Please try again shortly.';
      });
    } catch (e) {
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _login() async {
    ...
    try {
      ...
      if (role != 'applicant') {
        await supabase.auth.signOut();
        throw Exception(
          'This mobile app is for applicant accounts only.',
        );
      }
  ...
  String _friendlyAuthError(String message) {
    ...
    if (raw.contains('email not confirmed') || raw.contains('not confirmed')) {
      return 'Your email is not confirmed yet. Check your inbox for the verification message.';
    }
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'Need an account to apply for scholarship?',
            ...
            TextButton(
              ...
              child: const Text('Register as Scholarship Applicant'),
            ),
            TextButton(
              onPressed: _loading ? null : () => context.push('/verify-email-otp'),
